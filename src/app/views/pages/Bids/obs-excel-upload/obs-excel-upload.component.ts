...
@Component({
  selector: 'app-obs-excel-upload',
  templateUrl: './obs-excel-upload.component.html',
  styleUrls: ['./obs-excel-upload.component.scss']
})
export class OBSExcelUploadComponent implements OnInit {
  ...
  UploadCSV() {
   ...
   // formData.append('bid_winner_id',  bid_winner_id);
    ...
    Check = true
    if (Check) {
      this.FileUpService.uploadOBSExcel(formData).subscribe(data => {
        if(data instanceof HttpHeaderResponse)
        {
          return false
        }
        ...
        Swal.fire({
          icon:"error",
          text:error.error.data.data.message
        })
